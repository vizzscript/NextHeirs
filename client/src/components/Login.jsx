...
const Login = () => {
  ...
  const handleLogin = () => {
    if (!isRegistered) {
      setError('Registration is required before login');
      return;
    }
          ...
          <Form className='space-y-4 md:space-y-4'>
            ...
            {!isRegistered && (
              <p className="text-sm text-green-950 font-semibold">
                Registration is required before login. <Link to='/registration' onClick={handleRegistration} className="text-green-700">Register here</Link>
              </p>
            )}
          </Form>
        </div>
